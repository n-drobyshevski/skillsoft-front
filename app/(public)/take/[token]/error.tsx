...
/**
 * Error Boundary for Test Access Section.
 *
 * Catches unhandled errors in the test access token validation pages and provides
 ...
 */
export default function TestAccessError({
  error,
  reset,
}: {
  error: Error & {
  ...
  useEffect(() => {
    ...
    console.error('Test access page error:', {
      name: error.name,
  ...
  const errorInfo = useMemo(() => {
    ...
    if (lowerMessage.includes('token') || lowerMessage.includes('access') || lowerMessage.includes('expired') || lowerMessage.includes('invalid')) {
      return {
