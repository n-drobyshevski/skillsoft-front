...
/**
 * Error Boundary for Test Taking Section.
 *
 * Catches unhandled errors in the public test-taking pages and provides
 ...
 */
export default function TestTakingError({
  error,
  reset,
}: {
  error: Error & {
  ...
  useEffect(() => {
    ...
    console.error('Test taking page error:', {
      name: error.name,
  ...
  const errorInfo = useMemo(() => {
    ...
    if (lowerMessage.includes('test') || lowerMessage.includes('тест') || lowerMessage.includes('session') || lowerMessage.includes('сессия')) {
      return {
