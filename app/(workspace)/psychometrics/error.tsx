...
/**
 * Error Boundary for Psychometrics Section.
 *
 * Catches unhandled errors in psychometrics pages and provides
 ...
 */
export default function PsychometricsError({
  error,
  reset,
}: {
  error: Error & {
  ...
  useEffect(() => {
    ...
    console.error('Psychometrics page error:', {
      name: error.name,
  ...
  const errorInfo = useMemo(() => {
    ...
    if (lowerMessage.includes('psychometric') || lowerMessage.includes('reliability') || lowerMessage.includes('alpha')) {
      return {
        category: ErrorCategory.SERVER,
        title: 'Ошибка загрузки данных',
        message: 'Не удалось загрузить психометрические данные. Попробуйте обновить страницу.',
        isRetryable: true,
        suggestedAction: ErrorAction.RETRY,
      };
    }

    if (lowerMessage.includes('competenc') || lowerMessage.includes('компетенц')) {
      return {
        category: ErrorCategory.NOT_FOUND,
        title: 'Компетенция не найдена',
        message: 'Запрашиваемая компетенция не найдена или была удалена.',
        isRetryable: false,
        suggestedAction: ErrorAction.GO_BACK,
      };
    }

    if (lowerMessage.includes('item') || lowerMessage.includes('question') || lowerMessage.includes('вопрос')) {
      return {
        category: ErrorCategory.NOT_FOUND,
        title: 'Элемент не найден',
        message: 'Запрашиваемый элемент теста не найден или был удалён.',
        isRetryable: false,
        suggestedAction: ErrorAction.GO_BACK,
      };
    }
    ...
    <div className="flex flex-1 flex-col gap-4 p-4 pt-6 md:gap-6 md:p-6 items-center justify-center min-h-[400px]">
      <Card className="max-w-md w-full border-destructive/50">
        ...
        <CardFooter className="flex flex-col gap-2">
          ...
          {/* Psychometrics Dashboard Link */}
          <Button variant="outline" asChild className="w-full">
            <Link href="/psychometrics">
              <Activity className="w-4 h-4 mr-2" />
              К панели психометрики
            </Link>
          </Button>
