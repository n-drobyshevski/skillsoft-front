...
/**
 * Error Boundary for Skill Mapper Section.
 *
 * Catches unhandled errors in skill mapper pages and provides
 ...
 */
export default function SkillMapperError({
  error,
  reset,
}: {
  error: Error & {
  ...
  useEffect(() => {
    ...
    console.error('Skill mapper page error:', {
      name: error.name,
  ...
  const errorInfo = useMemo(() => {
    ...
    if (lowerMessage.includes('skill') || lowerMessage.includes('навык') || lowerMessage.includes('competenc') || lowerMessage.includes('компетенц')) {
      return {
