...
import { testResultsApi, competenciesApi } from '@/services/api';
  ...
  return (
    <div className="min-h-screen bg-muted/30 py-8">
      <div className="container max-w-4xl mx-auto px-4">
        ...
        <Card className={isPassed ? 'border-green-500/50' : 'border-amber-500/50'}>
          ...
          <CardContent className="pt-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <StatCard
                ...
                label="Баллы"
                value={`${result.overallScore} / ${Math.round(result.overallScore / (result.overallPercentage / 100))}`}
              />
              <StatCard
          ...
          <CardFooter className="flex flex-col sm:flex-row gap-3 pt-6">
            ...
            <Button asChild className="w-full sm:w-auto">
              <Link href={`/test-templates/${result.templateId}`}>
        ...
        {result.competencyScores && result.competencyScores.length > 0 && (
          <Card className="mt-6">
            <CardHeader>
              ...
              <CardDescription>
                Подробная статистика по каждой оцениваемой компетенции
              </CardDescription>
            </CardHeader>
        ...
        {/* Detailed breakdown (collapsible in future) */}
        <Card className="mt-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            {isPassed ? (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Отличная работа! Вы продемонстрировали хороший уровень владения оцениваемыми компетенциями.
                </p>
                <p className="text-sm text-muted-foreground">
                  Для дальнейшего развития рекомендуем обратить внимание на компетенции с наименьшим процентом.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Не расстраивайтесь! Используйте этот результат как возможность для роста.
                </p>
                <p className="text-sm text-muted-foreground">
                  Обратите особое внимание на компетенции, где ваш результат ниже 60%. 
                  Изучите дополнительные материалы и попробуйте пройти тест ещё раз.
                </p>
              </div>
            )}
          </CardContent>
